...
def x2o_get_qsfps():
    ...
    qsfp = qsfp_module()
    qsfp.configure(0, 0)
    qsfp.autodetect_optics()
    qsfps = qsfp.devices['optics']
    return qsfps
...
if __name__ == '__main__':
    ...
    if args.configure_vitex_100g is not None:
        ...
        if cage not in qsfps:
            ...
            #set_qsfp_rx_amplitude(qsfps[cage], 3) #3 is not valid according to the registers, but it does work, and performs better than 1
            set_qsfp_rx_emphasis(qsfps[cage], 0)
            set_qsfp_rx_amplitude(qsfps[cage], 2)
        ...
        for i in range(X2O_NUM_CAGES):
            if i in qsfps.keys():
                ...
                if type == "QSFP28" and vendor == "Vitex":
                    ...
                    set_qsfp_rx_amplitude(qsfp, 1) #3
